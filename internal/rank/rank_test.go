...
func TestRankDecisionTable(t *testing.T) {
	...
	rows := []struct {
		...
		{name: "caller ssh options unfit for mosh remove mosh-mux",
			caps: with(full, func(c *Caps) { c.MoshUnfit = "-L needs the ssh session mosh closes" }), link: steady, prefs: prefs(ModeAuto),
			want: "native-mux,ssh-raw,ssh", filtered: "mosh-mux", reason: "mosh-mux filtered: -L needs the ssh session mosh closes"},
		{name: "caller ssh options unfit for a domain remove both native tiers",
			caps: with(full, func(c *Caps) { c.NativeUnfit = "ssh options do not reach a wezterm domain" }), link: steady, prefs: prefs(ModeAuto),
			want: "mosh-mux,ssh", filtered: "native-mux,ssh-raw", reason: "ssh-raw filtered: ssh options do not reach a wezterm domain"},
